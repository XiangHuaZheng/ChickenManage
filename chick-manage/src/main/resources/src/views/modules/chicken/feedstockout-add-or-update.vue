    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.outDate) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedstockout/info/${this.dataForm.outDate}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.feedstockout.outNo
                this.dataForm.outCategory = data.feedstockout.outCategory
                this.dataForm.supplierName = data.feedstockout.supplierName
                this.dataForm.stockInNo = data.feedstockout.stockInNo
                this.dataForm.name = data.feedstockout.name
                this.dataForm.category = data.feedstockout.category
                this.dataForm.specifications = data.feedstockout.specifications
                this.dataForm.unit = data.feedstockout.unit
                this.dataForm.unitNum = data.feedstockout.unitNum
                this.dataForm.unitPrice = data.feedstockout.unitPrice
                this.dataForm.price = data.feedstockout.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedstockout/${!this.dataForm.outDate ? 'save' : 'update'}`),
              method: 'post',
