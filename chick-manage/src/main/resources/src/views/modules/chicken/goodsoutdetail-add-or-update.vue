    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsoutdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.goodsoutdetail.outNo
                this.dataForm.detailNo = data.goodsoutdetail.detailNo
                this.dataForm.name = data.goodsoutdetail.name
                this.dataForm.category = data.goodsoutdetail.category
                this.dataForm.specifications = data.goodsoutdetail.specifications
                this.dataForm.unit = data.goodsoutdetail.unit
                this.dataForm.unitNum = data.goodsoutdetail.unitNum
                this.dataForm.unitPrice = data.goodsoutdetail.unitPrice
                this.dataForm.price = data.goodsoutdetail.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsoutdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
