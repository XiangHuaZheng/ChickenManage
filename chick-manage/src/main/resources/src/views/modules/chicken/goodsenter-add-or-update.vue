    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsenter/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterNo = data.goodsenter.enterNo
                this.dataForm.enterDate = data.goodsenter.enterDate
                this.dataForm.enterCategory = data.goodsenter.enterCategory
                this.dataForm.supplierNo = data.goodsenter.supplierNo
                this.dataForm.supplierName = data.goodsenter.supplierName
                this.dataForm.agent = data.goodsenter.agent
                this.dataForm.sumPay = data.goodsenter.sumPay
                this.dataForm.hasPay = data.goodsenter.hasPay
                this.dataForm.noPay = data.goodsenter.noPay
                this.dataForm.remarks = data.goodsenter.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsenter/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
