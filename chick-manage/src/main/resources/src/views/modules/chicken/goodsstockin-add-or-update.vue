    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.inDate) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsstockin/info/${this.dataForm.inDate}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.inNo = data.goodsstockin.inNo
                this.dataForm.inCategory = data.goodsstockin.inCategory
                this.dataForm.supplierName = data.goodsstockin.supplierName
                this.dataForm.stockInNo = data.goodsstockin.stockInNo
                this.dataForm.name = data.goodsstockin.name
                this.dataForm.category = data.goodsstockin.category
                this.dataForm.specifications = data.goodsstockin.specifications
                this.dataForm.unit = data.goodsstockin.unit
                this.dataForm.unitNum = data.goodsstockin.unitNum
                this.dataForm.unitPrice = data.goodsstockin.unitPrice
                this.dataForm.price = data.goodsstockin.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsstockin/${!this.dataForm.inDate ? 'save' : 'update'}`),
              method: 'post',
