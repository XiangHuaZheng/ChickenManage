    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/materialenterdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterNo = data.materialenterdetail.enterNo
                this.dataForm.detailNo = data.materialenterdetail.detailNo
                this.dataForm.name = data.materialenterdetail.name
                this.dataForm.category = data.materialenterdetail.category
                this.dataForm.specifications = data.materialenterdetail.specifications
                this.dataForm.unit = data.materialenterdetail.unit
                this.dataForm.unitNum = data.materialenterdetail.unitNum
                this.dataForm.unitPrice = data.materialenterdetail.unitPrice
                this.dataForm.price = data.materialenterdetail.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/materialenterdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
