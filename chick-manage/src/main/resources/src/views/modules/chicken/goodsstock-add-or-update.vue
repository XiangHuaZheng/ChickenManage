    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsstock/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.goodsstock.name
                this.dataForm.category = data.goodsstock.category
                this.dataForm.specifications = data.goodsstock.specifications
                this.dataForm.unitPrice = data.goodsstock.unitPrice
                this.dataForm.unit = data.goodsstock.unit
                this.dataForm.stockNum = data.goodsstock.stockNum
                this.dataForm.stockIn = data.goodsstock.stockIn
                this.dataForm.stockOut = data.goodsstock.stockOut
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsstock/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
