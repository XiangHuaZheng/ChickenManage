    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/hatch/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.batchNo = data.hatch.batchNo
                this.dataForm.caseNo = data.hatch.caseNo
                this.dataForm.caseTime = data.hatch.caseTime
                this.dataForm.eggNum = data.hatch.eggNum
                this.dataForm.fertilizationRate = data.hatch.fertilizationRate
                this.dataForm.chikenNum = data.hatch.chikenNum
                this.dataForm.chickenRate = data.hatch.chickenRate
                this.dataForm.chickenTime = data.hatch.chickenTime
                this.dataForm.remarks = data.hatch.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/hatch/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
