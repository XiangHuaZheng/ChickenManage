    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedstock/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.name = data.feedstock.name
                this.dataForm.category = data.feedstock.category
                this.dataForm.specifications = data.feedstock.specifications
                this.dataForm.unitPrice = data.feedstock.unitPrice
                this.dataForm.unit = data.feedstock.unit
                this.dataForm.stockNum = data.feedstock.stockNum
                this.dataForm.stockIn = data.feedstock.stockIn
                this.dataForm.stockOut = data.feedstock.stockOut
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedstock/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
