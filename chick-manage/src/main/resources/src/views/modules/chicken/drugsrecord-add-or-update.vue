    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsrecord/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.chickenName = data.drugsrecord.chickenName
                this.dataForm.category = data.drugsrecord.category
                this.dataForm.dailyDate = data.drugsrecord.dailyDate
                this.dataForm.feedsName = data.drugsrecord.feedsName
                this.dataForm.raiseNo = data.drugsrecord.raiseNo
                this.dataForm.specifications = data.drugsrecord.specifications
                this.dataForm.drugNum = data.drugsrecord.drugNum
                this.dataForm.unit = data.drugsrecord.unit
                this.dataForm.unitPrice = data.drugsrecord.unitPrice
                this.dataForm.price = data.drugsrecord.price
                this.dataForm.breeder = data.drugsrecord.breeder
                this.dataForm.remarks = data.drugsrecord.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsrecord/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
