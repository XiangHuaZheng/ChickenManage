    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/materialoutdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.materialoutdetail.outNo
                this.dataForm.detailNo = data.materialoutdetail.detailNo
                this.dataForm.name = data.materialoutdetail.name
                this.dataForm.category = data.materialoutdetail.category
                this.dataForm.specifications = data.materialoutdetail.specifications
                this.dataForm.unit = data.materialoutdetail.unit
                this.dataForm.unitNum = data.materialoutdetail.unitNum
                this.dataForm.unitPrice = data.materialoutdetail.unitPrice
                this.dataForm.price = data.materialoutdetail.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/materialoutdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
