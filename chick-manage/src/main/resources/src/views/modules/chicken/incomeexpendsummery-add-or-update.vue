    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.category) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/incomeexpendsummery/info/${this.dataForm.category}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.incomePrice = data.incomeexpendsummery.incomePrice
                this.dataForm.expendPrice = data.incomeexpendsummery.expendPrice
                this.dataForm.profit = data.incomeexpendsummery.profit
                this.dataForm.beginTime = data.incomeexpendsummery.beginTime
                this.dataForm.endTime = data.incomeexpendsummery.endTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/incomeexpendsummery/${!this.dataForm.category ? 'save' : 'update'}`),
              method: 'post',
