    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsenterdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterNo = data.goodsenterdetail.enterNo
                this.dataForm.detailNo = data.goodsenterdetail.detailNo
                this.dataForm.name = data.goodsenterdetail.name
                this.dataForm.category = data.goodsenterdetail.category
                this.dataForm.specifications = data.goodsenterdetail.specifications
                this.dataForm.unit = data.goodsenterdetail.unit
                this.dataForm.unitNum = data.goodsenterdetail.unitNum
                this.dataForm.unitPrice = data.goodsenterdetail.unitPrice
                this.dataForm.price = data.goodsenterdetail.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsenterdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
