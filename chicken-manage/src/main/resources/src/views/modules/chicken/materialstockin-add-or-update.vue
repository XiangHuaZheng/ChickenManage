    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.inDate) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/materialstockin/info/${this.dataForm.inDate}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.inNo = data.materialstockin.inNo
                this.dataForm.inCategory = data.materialstockin.inCategory
                this.dataForm.supplierName = data.materialstockin.supplierName
                this.dataForm.stockInNo = data.materialstockin.stockInNo
                this.dataForm.name = data.materialstockin.name
                this.dataForm.category = data.materialstockin.category
                this.dataForm.specifications = data.materialstockin.specifications
                this.dataForm.unit = data.materialstockin.unit
                this.dataForm.unitNum = data.materialstockin.unitNum
                this.dataForm.unitPrice = data.materialstockin.unitPrice
                this.dataForm.price = data.materialstockin.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/materialstockin/${!this.dataForm.inDate ? 'save' : 'update'}`),
              method: 'post',
