    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsenterdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterNo = data.drugsenterdetail.enterNo
                this.dataForm.detailNo = data.drugsenterdetail.detailNo
                this.dataForm.name = data.drugsenterdetail.name
                this.dataForm.category = data.drugsenterdetail.category
                this.dataForm.specifications = data.drugsenterdetail.specifications
                this.dataForm.unit = data.drugsenterdetail.unit
                this.dataForm.unitNum = data.drugsenterdetail.unitNum
                this.dataForm.unitPrice = data.drugsenterdetail.unitPrice
                this.dataForm.price = data.drugsenterdetail.price
                this.dataForm.expirationDate = data.drugsenterdetail.expirationDate
                this.dataForm.remarks = data.drugsenterdetail.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsenterdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
