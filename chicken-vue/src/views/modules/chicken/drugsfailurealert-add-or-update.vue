...
<script>
  export default {
    ...
    methods: {
      init (id) {
        this.dataForm.failureDate = id || 0
        this.visible = true
        this.$nextTick(() => {
          this.$refs['dataForm'].resetFields()
          if (this.dataForm.failureDate) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsfailurealert/info/${this.dataForm.failureDate}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.inNo = data.drugsfailurealert.inNo
                this.dataForm.inDate = data.drugsfailurealert.inDate
                this.dataForm.inCategory = data.drugsfailurealert.inCategory
                this.dataForm.supplierName = data.drugsfailurealert.supplierName
                this.dataForm.stockInNo = data.drugsfailurealert.stockInNo
                this.dataForm.name = data.drugsfailurealert.name
                this.dataForm.category = data.drugsfailurealert.category
                this.dataForm.specifications = data.drugsfailurealert.specifications
                this.dataForm.unit = data.drugsfailurealert.unit
                this.dataForm.unitNum = data.drugsfailurealert.unitNum
                this.dataForm.unitPrice = data.drugsfailurealert.unitPrice
                this.dataForm.price = data.drugsfailurealert.price
                this.dataForm.remarks = data.drugsfailurealert.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsfailurealert/${!this.dataForm.failureDate ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                'failureDate': this.dataForm.failureDate || undefined,
