    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsoutdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.drugsoutdetail.outNo
                this.dataForm.detailNo = data.drugsoutdetail.detailNo
                this.dataForm.name = data.drugsoutdetail.name
                this.dataForm.category = data.drugsoutdetail.category
                this.dataForm.specifications = data.drugsoutdetail.specifications
                this.dataForm.unit = data.drugsoutdetail.unit
                this.dataForm.unitNum = data.drugsoutdetail.unitNum
                this.dataForm.unitPrice = data.drugsoutdetail.unitPrice
                this.dataForm.price = data.drugsoutdetail.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsoutdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
