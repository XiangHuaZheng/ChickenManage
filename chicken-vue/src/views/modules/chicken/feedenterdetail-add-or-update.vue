    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedenterdetail/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterNo = data.feedenterdetail.enterNo
                this.dataForm.detailNo = data.feedenterdetail.detailNo
                this.dataForm.name = data.feedenterdetail.name
                this.dataForm.category = data.feedenterdetail.category
                this.dataForm.specifications = data.feedenterdetail.specifications
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedenterdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
