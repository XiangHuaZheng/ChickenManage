    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsenter/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.enterNo = data.drugsenter.enterNo
                this.dataForm.enterDate = data.drugsenter.enterDate
                this.dataForm.enterCategory = data.drugsenter.enterCategory
                this.dataForm.supplierNo = data.drugsenter.supplierNo
                this.dataForm.supplierName = data.drugsenter.supplierName
                this.dataForm.agent = data.drugsenter.agent
                this.dataForm.sumPay = data.drugsenter.sumPay
                this.dataForm.hasPay = data.drugsenter.hasPay
                this.dataForm.noPay = data.drugsenter.noPay
                this.dataForm.remarks = data.drugsenter.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsenter/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
