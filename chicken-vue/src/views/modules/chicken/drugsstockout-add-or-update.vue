    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.outDate) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsstockout/info/${this.dataForm.outDate}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.drugsstockout.outNo
                this.dataForm.outCategory = data.drugsstockout.outCategory
                this.dataForm.supplierName = data.drugsstockout.supplierName
                this.dataForm.stockInNo = data.drugsstockout.stockInNo
                this.dataForm.name = data.drugsstockout.name
                this.dataForm.category = data.drugsstockout.category
                this.dataForm.specifications = data.drugsstockout.specifications
                this.dataForm.unit = data.drugsstockout.unit
                this.dataForm.unitNum = data.drugsstockout.unitNum
                this.dataForm.unitPrice = data.drugsstockout.unitPrice
                this.dataForm.price = data.drugsstockout.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsstockout/${!this.dataForm.outDate ? 'save' : 'update'}`),
              method: 'post',
