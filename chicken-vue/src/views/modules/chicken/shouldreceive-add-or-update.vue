    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.name) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/shouldreceive/info/${this.dataForm.name}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.category = data.shouldreceive.category
                this.dataForm.time = data.shouldreceive.time
                this.dataForm.incomePrice = data.shouldreceive.incomePrice
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/shouldreceive/${!this.dataForm.name ? 'save' : 'update'}`),
              method: 'post',
