    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsout/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.goodsout.outNo
                this.dataForm.outDate = data.goodsout.outDate
                this.dataForm.outCategory = data.goodsout.outCategory
                this.dataForm.supplierNo = data.goodsout.supplierNo
                this.dataForm.supplierName = data.goodsout.supplierName
                this.dataForm.agent = data.goodsout.agent
                this.dataForm.sumPay = data.goodsout.sumPay
                this.dataForm.hasPay = data.goodsout.hasPay
                this.dataForm.noPay = data.goodsout.noPay
                this.dataForm.remarks = data.goodsout.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsout/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
