    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/raiserecord/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.chickenName = data.raiserecord.chickenName
                this.dataForm.category = data.raiserecord.category
                this.dataForm.dailyDate = data.raiserecord.dailyDate
                this.dataForm.feedsName = data.raiserecord.feedsName
                this.dataForm.raiseNo = data.raiserecord.raiseNo
                this.dataForm.specifications = data.raiserecord.specifications
                this.dataForm.raiseNum = data.raiserecord.raiseNum
                this.dataForm.unit = data.raiserecord.unit
                this.dataForm.unitPrice = data.raiserecord.unitPrice
                this.dataForm.price = data.raiserecord.price
                this.dataForm.breeder = data.raiserecord.breeder
                this.dataForm.remarks = data.raiserecord.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/raiserecord/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
