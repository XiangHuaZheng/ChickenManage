    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/eggsstockout/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.eggsstockout.outNo
                this.dataForm.outDate = data.eggsstockout.outDate
                this.dataForm.outCategory = data.eggsstockout.outCategory
                this.dataForm.supplierNo = data.eggsstockout.supplierNo
                this.dataForm.supplierName = data.eggsstockout.supplierName
                this.dataForm.agent = data.eggsstockout.agent
                this.dataForm.chickenNo = data.eggsstockout.chickenNo
                this.dataForm.category = data.eggsstockout.category
                this.dataForm.sellNum = data.eggsstockout.sellNum
                this.dataForm.eggsPrice = data.eggsstockout.eggsPrice
                this.dataForm.sumPay = data.eggsstockout.sumPay
                this.dataForm.hasPay = data.eggsstockout.hasPay
                this.dataForm.noPay = data.eggsstockout.noPay
                this.dataForm.remarks = data.eggsstockout.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/eggsstockout/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
