<template>
  <el-dialog
    ...
    <el-form-item label="喂养量" prop="raiseNum">
      <el-input v-model="dataForm.raiseNum" placeholder="喂养量"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          raiseNum: [
            { required: true, message: '喂养量不能为空', trigger: 'blur' }
          ],
          unit: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/raiserecordsummary/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.chickenName = data.raiserecordsummary.chickenName
                this.dataForm.category = data.raiserecordsummary.category
                this.dataForm.dailyDate = data.raiserecordsummary.dailyDate
                this.dataForm.feedsName = data.raiserecordsummary.feedsName
                this.dataForm.raiseNo = data.raiserecordsummary.raiseNo
                this.dataForm.specifications = data.raiserecordsummary.specifications
                this.dataForm.raiseNum = data.raiserecordsummary.raiseNum
                this.dataForm.unit = data.raiserecordsummary.unit
                this.dataForm.unitPrice = data.raiserecordsummary.unitPrice
                this.dataForm.price = data.raiserecordsummary.price
                this.dataForm.breeder = data.raiserecordsummary.breeder
                this.dataForm.remarks = data.raiserecordsummary.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/raiserecordsummary/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                ...
                'raiseNum': this.dataForm.raiseNum,
