    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/dierecordsummary/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.chickenName = data.dierecordsummary.chickenName
                this.dataForm.category = data.dierecordsummary.category
                this.dataForm.dailyDate = data.dierecordsummary.dailyDate
                this.dataForm.dieNum = data.dierecordsummary.dieNum
                this.dataForm.dieReason = data.dierecordsummary.dieReason
                this.dataForm.remarks = data.dierecordsummary.remarks
                this.dataForm.recorder = data.dierecordsummary.recorder
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/dierecordsummary/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
