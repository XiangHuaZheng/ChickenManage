    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.outDate) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsstockout/info/${this.dataForm.outDate}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.goodsstockout.outNo
                this.dataForm.outCategory = data.goodsstockout.outCategory
                this.dataForm.supplierName = data.goodsstockout.supplierName
                this.dataForm.stockInNo = data.goodsstockout.stockInNo
                this.dataForm.name = data.goodsstockout.name
                this.dataForm.category = data.goodsstockout.category
                this.dataForm.specifications = data.goodsstockout.specifications
                this.dataForm.unit = data.goodsstockout.unit
                this.dataForm.unitNum = data.goodsstockout.unitNum
                this.dataForm.unitPrice = data.goodsstockout.unitPrice
                this.dataForm.price = data.goodsstockout.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/goodsstockout/${!this.dataForm.outDate ? 'save' : 'update'}`),
              method: 'post',
