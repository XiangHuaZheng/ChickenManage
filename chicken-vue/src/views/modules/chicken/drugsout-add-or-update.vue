<template>
  <el-dialog
    ...
    <el-form-item label="入库日期" prop="outDate">
      <el-input v-model="dataForm.outDate" placeholder="入库日期"></el-input>
    </el-form-item>
    <el-form-item label="入库类型" prop="outCategory">
      <el-input v-model="dataForm.outCategory" placeholder="入库类型"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsout/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.outNo = data.drugsout.outNo
                this.dataForm.outDate = data.drugsout.outDate
                this.dataForm.outCategory = data.drugsout.outCategory
                this.dataForm.supplierNo = data.drugsout.supplierNo
                this.dataForm.supplierName = data.drugsout.supplierName
                this.dataForm.agent = data.drugsout.agent
                this.dataForm.sumPay = data.drugsout.sumPay
                this.dataForm.hasPay = data.drugsout.hasPay
                this.dataForm.noPay = data.drugsout.noPay
                this.dataForm.remarks = data.drugsout.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/drugsout/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
