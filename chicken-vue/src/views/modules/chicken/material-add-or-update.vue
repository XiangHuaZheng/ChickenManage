<template>
  <el-dialog
    ...
    <el-form-item label="编号" prop="materialNo">
      <el-input v-model="dataForm.materialNo" placeholder="编号"></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/material/info/${this.dataForm.id}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.category = data.material.category
                this.dataForm.specifications = data.material.specifications
                this.dataForm.unit = data.material.unit
                this.dataForm.unitPrice = data.material.unitPrice
                this.dataForm.remarks = data.material.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/material/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
