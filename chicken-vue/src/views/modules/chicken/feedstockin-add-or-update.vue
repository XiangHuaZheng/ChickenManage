    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.inDate) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedstockin/info/${this.dataForm.inDate}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.inNo = data.feedstockin.inNo
                this.dataForm.inCategory = data.feedstockin.inCategory
                this.dataForm.supplierName = data.feedstockin.supplierName
                this.dataForm.stockInNo = data.feedstockin.stockInNo
                this.dataForm.name = data.feedstockin.name
                this.dataForm.category = data.feedstockin.category
                this.dataForm.specifications = data.feedstockin.specifications
                this.dataForm.unit = data.feedstockin.unit
                this.dataForm.unitNum = data.feedstockin.unitNum
                this.dataForm.unitPrice = data.feedstockin.unitPrice
                this.dataForm.price = data.feedstockin.price
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/feedstockin/${!this.dataForm.inDate ? 'save' : 'update'}`),
              method: 'post',
