    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/hatchrecord/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.hatchTime = data.hatchrecord.hatchTime
                this.dataForm.hatchTemp = data.hatchrecord.hatchTemp
                this.dataForm.hatchHumidity = data.hatchrecord.hatchHumidity
                this.dataForm.doorLocation = data.hatchrecord.doorLocation
                this.dataForm.numLocation = data.hatchrecord.numLocation
                this.dataForm.operation = data.hatchrecord.operation
                this.dataForm.machineNo = data.hatchrecord.machineNo
                this.dataForm.duty = data.hatchrecord.duty
                this.dataForm.changeDutyTime = data.hatchrecord.changeDutyTime
                this.dataForm.remarks = data.hatchrecord.remarks
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/chicken/hatchrecord/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
